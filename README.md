...
C37.118 gateway adapters can reach its five-PMU legacy V2 fixture. It has no Kafka,
...
conservative synchronization-uncertain STAT. The five-PMU V2 C37.118 gateway profile
is the explicit exception: PMU IDs `1001` and `1002` emit STAT `0`, so the
C37.118 gateway adapters normalize them as `quality.valid=true`; PMU IDs `1003`
through `1005` remain conservative-invalid. This is controlled integration-fixture
...
The default five-PMU C37.118 gateway profile exposes five internal endpoints at
`172.30.0.10:4712` through `172.30.0.10:4716`, matching gateway catalog sources
`pmu-bay-01` through `pmu-bay-05` and stream and PMU IDs `1001` through `1005`.
